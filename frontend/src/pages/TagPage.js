    ...
    return (
        <PtPageShell testid="tag-page">
            ...
            <div className="px-4 lg:px-5 py-2 hairline-b flex items-center gap-1">
                {SORTS.map((s) => (
                    <button key={s.key} onClick={() => setSort(s.key)} className={`px-3 py-1 rounded-full text-[12px] font-medium ${sort === s.key ? "chip-filter-on" : "bg-black/[0.04] text-black hover:bg-black/[0.08]"}`}>
                        {s.label}
                    </button>
                ))}
            </div>
