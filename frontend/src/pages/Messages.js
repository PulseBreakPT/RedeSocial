    ...
    return (
        <div data-testid="conversations-list">
            ...
            <div className="px-3 lg:px-4 pb-2 flex gap-1.5 overflow-x-auto scrollbar-hide">
                {FILTERS.map((f) => (
                    <button key={f.key} onClick={() => setFilter(f.key)} data-testid={`messages-filter-${f.key}`} className={`shrink-0 px-3 py-1.5 rounded-full text-[12px] font-medium ${filter === f.key ? "chip-filter-on" : "bg-black/[0.04] text-black hover:bg-black/[0.08]"}`}>{f.label}</button>
                ))}
            </div>
