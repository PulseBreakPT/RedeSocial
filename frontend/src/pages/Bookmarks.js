        ...
        <PtPageShell testid="bookmarks-page">
            ...
            >
                <div className="px-3 lg:px-4 pb-2 flex flex-wrap items-center gap-2">
                    ...
                    <div className="inline-flex bg-black/[0.04] rounded-full p-0.5">
                        {[
                            ...
                            return (
                                <button key={t.k} onClick={() => setType(t.k)} data-testid={`bookmarks-type-${t.k}`} className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-medium transition ${active ? "chip-filter-on" : "text-black hover:text-black"}`}>
                                    <Icon size={11} /> {t.l}
                                </button>
                            );
                        })}
                ...
                <div className="px-3 lg:px-4 pb-2.5 flex gap-1.5 overflow-x-auto scrollbar-hide hairline-t pt-2">
                    <button onClick={() => setActiveCol("")} data-testid="bookmarks-col-all" className={`shrink-0 px-3 py-1.5 rounded-full text-[12px] font-medium ${activeCol === "" ? "chip-filter-on" : "bg-black/[0.04] text-black hover:bg-black/[0.08]"}`}>
                        Todos
                    </button>
                    <button onClick={() => setActiveCol("uncategorized")} className={`shrink-0 px-3 py-1.5 rounded-full text-[12px] font-medium ${activeCol === "uncategorized" ? "chip-filter-on" : "bg-black/[0.04] text-black hover:bg-black/[0.08]"}`}>
                        Sem coleção
                    </button>
                    {collections.map((c) => (
                        <div key={c.id} className="shrink-0 inline-flex items-center gap-0.5">
                            <button onClick={() => setActiveCol(c.id)} data-testid={`bookmarks-col-${c.id}`} className={`px-3 py-1.5 rounded-l-full text-[12px] font-medium ${activeCol === c.id ? "chip-filter-on" : "bg-black/[0.04] text-black hover:bg-black/[0.08]"}`}>
