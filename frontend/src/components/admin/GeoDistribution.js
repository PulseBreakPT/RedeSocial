 ...
 */
export function GeoDistribution({ rows = [], hasData = true, totalUsers = 0 }) {
    if (!rows.length) {
        return (
    ...
    // Real data only — if 100% is "sem_regiao", display that honestly.
    return (
        <div>
            {rows.map((r) => (
                <div key={r.key} className={`ops-geo-row ${r.key === "sem_regiao" ? "ops-geo-row--warn" : ""}`}>
